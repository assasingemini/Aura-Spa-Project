...
import {
  ...
  Image, Settings, Sparkles, Menu, Bell, Search,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 border-b border-pink-100">
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#EC4899] to-[#A855F7] flex items-center justify-center shadow-lg shadow-pink-200/50 shrink-0">
        ...
        {SIDEBAR_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {sidebarOpen && active && <ChevronRight className="w-4 h-4 ml-auto text-pink-400/60 shrink-0" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-pink-100 space-y-2">
        ...
        >
          ...
          {sidebarOpen && <span className="text-sm whitespace-nowrap">View Website</span>}
        </Link>
        <button
          onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all"
        >
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-pink-100">
              ...
              <div className="hidden sm:block">
                <p className="text-gray-900 text-sm font-medium leading-none">{user?.name || user?.email?.split('@')[0] || "Admin"}</p>
                <p className="text-pink-400 text-xs mt-0.5 capitalize">{user?.role?.toLowerCase() || "Super Admin"}</p>
              </div>
            </div>
          </div>
        </header>
