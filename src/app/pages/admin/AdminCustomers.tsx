...
import { adminCustomers, Customer } from "../../data/mockData";
...
export function AdminCustomers() {
  ...
  const avgVisits = Math.round(customers.reduce((sum, c) => sum + c.totalVisits, 0) / customers.length);

  return (
    <div>
      <div className="mb-8">
        <h1 style={SERIF} className="text-gray-900 text-2xl font-semibold">Customer Management</h1>
        <p className="text-gray-400 text-sm mt-1">{customers.length} registered clients</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Clients", value: customers.length.toString(), icon: Users, color: "from-[#EC4899] to-[#F472B6]", bg: "from-pink-50 to-rose-50", border: "border-pink-200/60" },
          ...
          { label: "Platinum Members", value: customers.filter((c) => c.tier === "Platinum").length.toString(), icon: Award, color: "from-purple-500 to-purple-700", bg: "from-purple-50 to-fuchsia-50", border: "border-purple-200/60" },
        ].map((stat) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 backdrop-blur-md">
          <div className="w-full max-w-md rounded-2xl bg-white border border-pink-100 p-8 shadow-2xl shadow-pink-200/30">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setSelectedCustomer(null)} className="p-2 rounded-lg hover:bg-pink-50 text-gray-400 hover:text-pink-600 transition-all">✕</button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                ...
                <div key={stat.label} className="p-4 rounded-xl bg-pink-50/60 border border-pink-100 text-center">
                  <p style={SERIF} className="text-gray-900 font-semibold">{stat.value}</p>
